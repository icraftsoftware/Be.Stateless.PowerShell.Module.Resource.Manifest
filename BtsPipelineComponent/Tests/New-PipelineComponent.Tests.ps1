...
Import-Module -Name $PSScriptRoot\..\BtsPipelineComponent -Force

Describe 'New-PipelineComponent' {
    InModuleScope BtsPipelineComponent {

        Context 'When PipelineComponent file does not exist' {
            BeforeAll {
        ...
        Context 'When PipelineComponent file exists' {
            BeforeAll {
            ...
            It 'Returns a custom object with both a path and a name property.' {
                ...
                $actualItem = New-PipelineComponent -Path TestDrive:\one.txt -PassThru
            ...
            It 'Returns a collection of custom objects with both a path and a name property.' {
                ...
                $actualItems = New-PipelineComponent -Path (Get-ChildItem -Path TestDrive:\) -PassThru
        ...
        Context 'Creating PipelineComponents must be done via the ScriptBlock passed to New-Manifest' {
            BeforeAll {
                # create some empty files
                '' > TestDrive:\one.txt
                '' > TestDrive:\two.txt
            ...
            It 'Accumulates the Items as a PipelineComponents resource collection of the Manifest being built.' {
                $expectedItems = @(
                    [PSCustomObject]@{ Name = 'one.txt' ; Path = 'TestDrive:\one.txt' | Resolve-Path | Select-Object -ExpandProperty ProviderPath }
                ...
                $manifest = New-Manifest -Type Application -Name 'BizTalk.Factory' -Build {
                    New-PipelineComponent -Path (Get-ChildItem -Path TestDrive:\)
                }

                $manifest | Should -Not -BeNullOrEmpty
                $manifest.ContainsKey('PipelineComponents') | Should -BeTrue
                $manifest.PipelineComponents | Should -HaveCount 3
                0..2 | ForEach-Object -Process { Compare-Item -ReferenceItem $expectedItems[$_] -DifferenceItem $manifest.PipelineComponents[$_] | Should -BeNullOrEmpty }
            }
        }
