    ...
    InModuleScope BtsBinding {
        ...
        Context 'When binding file does not exist' {
            ...
            It 'Throws a ParameterBindingValidationException.' {
                { New-Binding -Path 'z:\binding.dll' } | Should -Throw -ExceptionType $ParameterBindingValidationExceptionType
            }
        }
        ...
        Context 'Creating Bindings must be done via the ScriptBlock passed to New-Manifest' {
            BeforeAll {
                # create some empty files
                '' > TestDrive:\one.txt
                '' > TestDrive:\two.txt
            ...
            It 'Accumulates the Items as a Bindings resource collection of the Manifest being built.' {
                $expectedItems = @(
                    [PSCustomObject]@{ Name = 'one.txt' ; Path = 'TestDrive:\one.txt' | Resolve-Path | Select-Object -ExpandProperty ProviderPath }
                ...
                $manifest.ContainsKey('Bindings') | Should -BeTrue
                $manifest.Bindings | Should -HaveCount 3
                0..2 | ForEach-Object -Process { Compare-Item -ReferenceItem $expectedItems[$_] -DifferenceItem $manifest.Bindings[$_] | Should -BeNullOrEmpty }
            }
        }
