   ...
   InModuleScope Resource.Manifest {
      ...
      Context 'Creating Installers must be done via the ScriptBlock passed to New-ResourceManifest' {
         ...
         It 'Accumulates Installers into the Manifest being built.' {
            ...
            $builtManifest.Installers | Should -HaveCount $expectedItems.Length
            0..($expectedItems.Length - 1) | ForEach-Object -Process { Compare-ResourceItem -ReferenceItem $expectedItems[$_] -DifferenceItem $builtManifest.Installers[$_] | Should -BeNullOrEmpty }
         }
      }
