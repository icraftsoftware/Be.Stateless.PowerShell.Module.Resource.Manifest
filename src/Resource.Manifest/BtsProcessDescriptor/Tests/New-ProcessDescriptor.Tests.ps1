   ...
   InModuleScope Resource.Manifest {
      ...
      Context 'Creating ProcessDescriptors must be done via the ScriptBlock passed to New-ResourceManifest' {
         ...
         It 'Accumulates ProcessDescriptors into the Manifest being built.' {
            ...
            $builtManifest.ProcessDescriptors | Should -HaveCount $expectedItems.Length
            0..($expectedItems.Length - 1) | ForEach-Object -Process { Compare-ResourceItem -ReferenceItem $expectedItems[$_] -DifferenceItem $builtManifest.ProcessDescriptors[$_] | Should -BeNullOrEmpty }
         }
      }
