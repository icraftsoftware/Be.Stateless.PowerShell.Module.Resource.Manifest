    ...
    InModuleScope Resource.Manifest {
        ...
        Context 'When ProcessDescriptor file exists' {
            ...
            It 'Returns a custom object with both a path and a name property.' {
                $expectedItem = [PSCustomObject]@{ DatabaseServer = 'server' ; Name = 'one.txt' ; Path = 'TestDrive:\one.txt' | Resolve-Path | Select-Object -ExpandProperty ProviderPath }

                $actualItem = New-ProcessDescriptor -Path TestDrive:\one.txt -DatabaseServer server -PassThru
            ...
            It 'Returns a collection of custom objects with both a path and a name property.' {
                $expectedItems = @(
                    [PSCustomObject]@{ DatabaseServer = 'server' ; Name = 'one.txt' ; Path = 'TestDrive:\one.txt' | Resolve-Path | Select-Object -ExpandProperty ProviderPath }
                    [PSCustomObject]@{ DatabaseServer = 'server' ; Name = 'two.txt' ; Path = 'TestDrive:\two.txt' | Resolve-Path | Select-Object -ExpandProperty ProviderPath }
                )

                $actualItems = New-ProcessDescriptor -Path (Get-ChildItem -Path TestDrive:\) -DatabaseServer server -PassThru
        ...
        Context 'Creating ProcessDescriptors must be done via the ScriptBlock passed to New-ResourceManifest' {
            ...
            It 'Accumulates ProcessDescriptors into the Manifest being built.' {
                $expectedItems = @(
                    [PSCustomObject]@{ DatabaseServer = 'server' ; Name = 'one.txt' ; Path = 'TestDrive:\one.txt' | Resolve-Path | Select-Object -ExpandProperty ProviderPath }
                    [PSCustomObject]@{ DatabaseServer = 'server' ; Name = 'six.txt' ; Path = 'TestDrive:\six.txt' | Resolve-Path | Select-Object -ExpandProperty ProviderPath }
                    [PSCustomObject]@{ DatabaseServer = 'server' ; Name = 'two.txt' ; Path = 'TestDrive:\two.txt' | Resolve-Path | Select-Object -ExpandProperty ProviderPath }
                )

                $builtManifest = New-ResourceManifest -Type Application -Name 'BizTalk.Factory' -Build {
                    New-ProcessDescriptor -Path (Get-ChildItem -Path TestDrive:\) -DatabaseServer server
                }
