...
Describe 'New-Pipeline' {
   InModuleScope Resource.Manifest {
      ...
      Context 'When Pipeline file does not exist' {
         BeforeAll {
         ...
         It 'Throws a ParameterBindingValidationException.' {
            { New-Pipeline -Path 'c:\Pipeline.dll' } | Should -Throw -ExceptionType $ParameterBindingValidationExceptionType
         }
      }

      Context 'When Pipeline file exists' {
         BeforeAll {
         ...
         It 'Returns a custom object with both a path and a name property.' {
            ...
            $actualItem = New-Pipeline -Path TestDrive:\one.txt -PassThru
         ...
         It 'Returns a collection of custom objects with both a path and a name property.' {
            ...
            $actualItems = New-Pipeline -Path (Get-ChildItem -Path TestDrive:\) -PassThru
      ...
      Context 'Creating Pipelines must be done via the ScriptBlock passed to New-ResourceManifest' {
         BeforeAll {
            # create some empty files
            '' > TestDrive:\one.txt
            '' > TestDrive:\two.txt
         ...
         It 'Accumulates Pipelines into the Manifest being built.' {
            ...
            $builtManifest.Pipelines | Should -HaveCount $expectedItems.Length
            0..($expectedItems.Length - 1) | ForEach-Object -Process { Compare-ResourceItem -ReferenceItem $expectedItems[$_] -DifferenceItem $builtManifest.Pipelines[$_] | Should -BeNullOrEmpty }
         }
      }
