   ...
   InModuleScope Resource.Manifest {
      ...
      Context 'Creating Maps must be done via the ScriptBlock passed to New-ResourceManifest' {
         ...
         It 'Accumulates Maps into the Manifest being built.' {
            ...
            $builtManifest.Maps | Should -HaveCount $expectedItems.Length
            0..($expectedItems.Length - 1) | ForEach-Object -Process { Compare-ResourceItem -ReferenceItem $expectedItems[$_] -DifferenceItem $builtManifest.Maps[$_] | Should -BeNullOrEmpty }
         }
      }
