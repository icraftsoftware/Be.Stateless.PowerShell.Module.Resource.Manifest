   ...
   $items | ForEach-Object -Process {
      ...
      if ($PassThru) {
         ...
         if ($Condition -is [ScriptBlock] -or $Condition) {
            if ($Manifest.ContainsKey($ResourceGroup)) {
               $Manifest.$ResourceGroup = @($Manifest.$ResourceGroup) + $item
            } else {
               $Manifest.Add($ResourceGroup, $item)
            }
         }
      }
   }
      ...
      if ($referenceProperties.Contains($key) -and !$differenceProperties.Contains($key)) {
         ...
         if ($referenceValue -is [array] -and $differenceValue -is [array]) {
            ...
            if ($arrayDifferences | Test-Any) {
               $uniqueReferenceValues = @($arrayDifferences | Where-Object -FilterScript { $_.SideIndicator -eq '<=' } | ForEach-Object -Process { $_.InputObject }) -join ', '
               $uniqueDifferenceValues = @($arrayDifferences | Where-Object -FilterScript { $_.SideIndicator -eq '=>' } | ForEach-Object -Process { $_.InputObject }) -join ', '
