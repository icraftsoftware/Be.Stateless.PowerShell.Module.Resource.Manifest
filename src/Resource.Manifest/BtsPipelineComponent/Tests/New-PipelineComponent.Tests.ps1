...
Describe 'New-PipelineComponent' {
   InModuleScope Resource.Manifest {
      ...
      Context 'Creating PipelineComponents must be done via the ScriptBlock passed to New-ResourceManifest' {
         BeforeAll {
            # create some empty files
            '' > TestDrive:\one.txt
            '' > TestDrive:\two.txt
         ...
         It 'Accumulates PipelineComponents into the Manifest being built.' {
            ...
            $builtManifest.PipelineComponents | Should -HaveCount $expectedItems.Length
            0..($expectedItems.Length - 1) | ForEach-Object -Process { Compare-ResourceItem -ReferenceItem $expectedItems[$_] -DifferenceItem $builtManifest.PipelineComponents[$_] | Should -BeNullOrEmpty }
         }
      }
