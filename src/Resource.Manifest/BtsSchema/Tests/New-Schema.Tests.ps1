...
Describe 'New-Schema' {
   InModuleScope Resource.Manifest {
      ...
      Context 'Creating Schemas must be done via the ScriptBlock passed to New-ResourceManifest' {
         BeforeAll {
            # create some empty files
            '' > TestDrive:\one.txt
            '' > TestDrive:\two.txt
         ...
         It 'Accumulates Schemas into the Manifest being built.' {
            ...
            $builtManifest.Schemas | Should -HaveCount $expectedItems.Length
            0..($expectedItems.Length - 1) | ForEach-Object -Process { Compare-ResourceItem -ReferenceItem $expectedItems[$_] -DifferenceItem $builtManifest.Schemas[$_] | Should -BeNullOrEmpty }
         }
      }
