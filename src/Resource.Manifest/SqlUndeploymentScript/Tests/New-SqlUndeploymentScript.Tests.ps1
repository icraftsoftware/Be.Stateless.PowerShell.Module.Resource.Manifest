   ...
   InModuleScope Resource.Manifest {
      ...
      Context 'When optional Variables are null or empty' {
         ...
         It 'Does not throw because Variables is null.' {
            { New-SqlUndeploymentScript -Path TestDrive:\one.sql -Server localhost -Variable $null -PassThru } |
               Should -Not -Throw
         }
         It 'Does not throw because Variables is empty.' {
            { New-SqlUndeploymentScript -Path TestDrive:\one.sql -Server localhost -Variable @{ } -PassThru } |
               Should -Not -Throw
         }
      }
