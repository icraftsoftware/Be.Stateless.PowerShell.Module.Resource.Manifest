...
Import-Module -Name $PSScriptRoot\..\BtsPipeline -Force
Import-Module -Name $PSScriptRoot\..\..\Resource -Force

Describe 'New-Pipeline' {
    InModuleScope BtsPipeline {
        ...
        Context 'When Pipeline file does not exist' {
            ...
            It 'Throws a ParameterBindingValidationException.' {
                { New-Pipeline -Path 'z:\Pipeline.dll' } | Should -Throw -ExceptionType $ParameterBindingValidationExceptionType
            }
        }
        ...
        Context 'Creating Pipelines must be done via the ScriptBlock passed to New-Manifest' {
            BeforeAll {
                # create some empty files
                '' > TestDrive:\one.txt
                '' > TestDrive:\two.txt
