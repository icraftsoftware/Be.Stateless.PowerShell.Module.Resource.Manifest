...
Describe 'New-Component' {
    InModuleScope Resource.Manifest {
        ...
        Context 'Creating Components must be done via the ScriptBlock passed to New-ResourceManifest' {
            BeforeAll {
                # create some empty files
                '' > TestDrive:\one.txt
                '' > TestDrive:\two.txt
            ...
            It 'Accumulates Components into the Manifest being built.' {
                ...
                0..2 | ForEach-Object -Process { Compare-ResourceItem -ReferenceItem $expectedItems[$_] -DifferenceItem $builtManifest.Components[$_] | Should -BeNullOrEmpty }
            }
        }
