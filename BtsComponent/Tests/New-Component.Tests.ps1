...
Describe 'New-Component' {
    InModuleScope BtsComponent {
        ...
        Context 'Creating Components must be done via the ScriptBlock passed to New-Manifest' {
            ...
            It 'Accumulates the Items as a Components resource collection of the Manifest being built.' {
                $expectedItems = @(
                    [PSCustomObject]@{ Name = 'one.txt' ; Path = 'TestDrive:\one.txt' | Resolve-Path | Select-Object -ExpandProperty ProviderPath }
                ...
                $manifest.ContainsKey('Components') | Should -BeTrue
                $manifest.Components | Should -HaveCount 3
                0..2 | ForEach-Object -Process { Compare-Item -ReferenceItem $expectedItems[$_] -DifferenceItem $manifest.Components[$_] | Should -BeNullOrEmpty }
            }
        }
