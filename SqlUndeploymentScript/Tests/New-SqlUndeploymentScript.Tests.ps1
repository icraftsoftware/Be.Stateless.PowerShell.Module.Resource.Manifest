    ...
    InModuleScope SqlUndeploymentScript {
        ...
        Context 'When SqlUndeploymentScript file does not exist' {
            ...
            It 'Throws a ParameterBindingValidationException.' {
                { New-SqlUndeploymentScript -Path 'z:\SqlUndeploymentScript.dll' } | Should -Throw -ExceptionType $ParameterBindingValidationExceptionType
            }
        }
        ...
        Context 'Creating SqlUndeploymentScripts must be done via the ScriptBlock passed to New-Manifest' {
            BeforeAll {
                # create some empty files
                '' > TestDrive:\one.sql
                '' > TestDrive:\two.sql
